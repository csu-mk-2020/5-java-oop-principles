...
public class Logger {
    ...
    private void log(LoggerLevel level, String message) {
        if (level.ordinal() >= this.level.ordinal()) {
            System.err.println("[" + this.level.toString() + "]" + dateFormat.format(new Date()) + this.name + " - " + message);
        }
    }

    private void log(LoggerLevel level, String format, Object... objects) {
        if (level.ordinal() >= this.level.ordinal()) {
            System.err.println("[" + this.level.toString() + "]" + dateFormat.format(new Date()) + this.name + " - " + String.format(format, objects));
        }
    }
